...
public final class SemesterDetailPanel extends AbstractCurriculumPanel implements EntityListener {
    ...
    private GridWidget initGridWidget(Class<? extends Entity> view,
        ...
        currentGW.setData(subjectsType);
        ...
        currentGW.addButtonClickListener(AbstractToolbar.ADD_BUTTON, new Button.ClickListener() {
            ...
            public void buttonClick(Button.ClickEvent event) {
                Message.showConfirm(getUILocaleUtil().getMessage("confirmation.save"), new AbstractYesButtonListener() {
                    @Override
                    public void buttonClick(Button.ClickEvent event) {
                        addSubjectsFromIups(curriculum, semester, currentGW);
                    }
                });
            }
        });
    ...
    private void addSubjectsFromIups(CURRICULUM curriculum, SEMESTER semester,
                                     GridWidget currentGW) {
        boolean isMainSubjects = currentGW.getData().equals(SubjectsType.MAIN_SUBJECTS);
        boolean isElectiveSubjects = currentGW.getData().equals(SubjectsType.ELECTIVE_SUBJECTS);
        try {
            ...
            sqlSB.append(" AND stu_edu.speciality_id = ?1 AND stu.diploma_type_id = ?2 " +
                    "      AND stu.entrance_year_id = ?3 AND sem_subj.semester_data_id = ?4 " +
                    "ORDER BY sem_subj.subject_id");
            ...
            params.put(4, CommonUtils.getSemesterDataBySemesterAndEntranceYear(semester,
                    curriculum.getEntranceYear()).getId().getId());
            ...
            if (isMainSubjects) {
                initCurriculumDetail(curriculum, semester, subjectIds);
            } else if (isElectiveSubjects) {
                initElectiveSubject(curriculum, semester, subjectIds);
            }
    ...
    private void initElectiveSubject(CURRICULUM curriculum, SEMESTER semester,
                                     List<Long> subjectIds) throws Exception {
        ...
        for (Long subjectId : subjectIds) {
            ...
            electiveSubject.setSemesterData(CommonUtils.
                    getSemesterDataBySemesterAndEntranceYear(semester, entranceYear));
    ...
    private void initCurriculumDetail(CURRICULUM curriculum, SEMESTER semester,
        ...
        for (Long subjectId : subjectIds) {
            ...
            curriculumDetail.setSubject(SessionFacadeFactory.getSessionFacade(
                    CommonEntityFacadeBean.class).lookup(SUBJECT.class, ID.valueOf(subjectId)));
            curriculumDetail.setSemesterData(CommonUtils.
                    getSemesterDataBySemesterAndEntranceYear(semester, entranceYear));
            SessionFacadeFactory.getSessionFacade(CommonEntityFacadeBean.class).create(
                    curriculumDetail);
        }
    ...
    public Integer getTotalCredit() {
        Integer mainSubjectsCredit = (Integer) ((DBGridModel) mainSubjectsGW.getWidgetModel()).getFooterValue("credit");
        ...
        Integer electiveSubjectsCredit = (Integer) ((DBGridModel) electiveSubjectsGW.getWidgetModel()).getFooterValue("credit");
