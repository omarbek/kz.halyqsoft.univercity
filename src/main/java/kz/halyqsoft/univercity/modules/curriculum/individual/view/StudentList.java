...
//        }
        ...
        if (sb.length() > 0) {
            sb.insert(0, " where ");
            String sql = "select a.ID, b.CODE, trim(b.LAST_NAME||' '||b.FIRST_NAME||' '||coalesce(b.MIDDLE_NAME, '')) FIO, c.CATEGORY_NAME, e.STATUS_NAME, f.DEPT_SHORT_NAME FACULTY, g.SPEC_NAME, decode(b.LOCKED, 0, null, h.REASON) LOCK_REASON from KBTU.T_STUDENT a inner join KBTU.T_USER b on a.ID = b.ID inner join KBTU.T_STUDENT_CATEGORY c on a.CATEGORY_ID = c.ID inner join KBTU.T_STUDENT_EDUCATION d on a.ID = d.STUDENT_ID and d.CHILD_ID is null inner join KBTU.T_STUDENT_STATUS e on d.STUDENT_STATUS_ID = e.ID inner join KBTU.DEPARMENT f on d.FACULTY_ID = f.ID inner join KBTU.SPECIALITY g on d.SPECIALITY_ID = g.ID left join KBTU.T_LOCK_REASON h on b.LOCK_REASON_ID = h.ID" + sb.toString() + " order by FIO";
            try {
