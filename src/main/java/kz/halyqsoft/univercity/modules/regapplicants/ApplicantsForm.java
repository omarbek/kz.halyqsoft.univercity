...
public final class ApplicantsForm extends UsersForm {
    ...
    protected VerticalLayout getMessForm(FormModel dataFM, Label mess) {
        ...
        try {
            ...
            StreamResource myResourceAnketa = createResourceStudent("178", student);
            FileDownloader fileDownloaderAnketa = new FileDownloader(myResourceAnketa);
            myResourceAnketa.setMIMEType("application/pdf");
            myResourceAnketa.setCacheTime(0);
            fileDownloaderAnketa.extend(downloadButtonRegisterButton);

            StreamResource myResourceZayav = createResourceStudent("179", student);
            FileDownloader fileDownloaderZayav = new FileDownloader(myResourceZayav);
            myResourceZayav.setMIMEType("application/pdf");
            myResourceZayav.setCacheTime(0);
            fileDownloaderZayav.extend(downloadButtonRegisterButton);
    ...
    public static StreamResource createResourceStudent(String value, STUDENT student) {
        ...
        } else if( value.equals("178")){
            fileName = "Анкета_" + Calendar.getInstance().getTimeInMillis() + ".pdf";
        } else if( value.equals("179")){
            fileName = "Заявление_" + Calendar.getInstance().getTimeInMillis() + ".pdf";
        } else {
