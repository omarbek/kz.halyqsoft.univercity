...
public class GroupsView extends AbstractTaskView implements FilterPanelListener, EntityListener {
    ...
    private static final String KAZAKH_LANGUAGE = "kz";
    ...
    private void generateNew(int STUDENT_MAX) {
        ...
        try {
            ...
            languageMap.put(KAZAKH_LANGUAGE, specialityMapKz);
            languageMap.put(RUSSIAN_LANGUAGE, specialityMapRu);
            languageMap.put(ENGLISH_LANGUAGE, specialityMapEn);
            ...
            for (STUDENT s : (List<STUDENT>) students) {
                ...
                if (lang.equalsIgnoreCase("Казахский")) {
                    languageMap.get(KAZAKH_LANGUAGE).get(getStudentEducationByStudentId(s.getId()).getSpeciality()).add(s);
                } else if (lang.equalsIgnoreCase("Английский")) {
                    languageMap.get(ENGLISH_LANGUAGE).get(getStudentEducationByStudentId(s.getId()).getSpeciality()).add(s);
                } else {
                    languageMap.get(RUSSIAN_LANGUAGE).get(getStudentEducationByStudentId(s.getId()).getSpeciality()).add(s);
                }
            }
                ...
                for (SPECIALITY speciality : languageMap.get(langKey).keySet()) {
                    ...
                    if (languageMap.get(langKey).get(speciality).size() > 0) {
                        ...
                        if (groupCounter == 1) {
                            ...
                            try {
                                groups.setStudyYear(SessionFacadeFactory.getSessionFacade(CommonEntityFacadeBean.class).
                                        lookup(STUDY_YEAR.class, ID.valueOf(1)));
                                groups.setLanguage(getLanguage(langKey));
    ...
    private LANGUAGE getLanguage(String langKey) throws Exception {
        ID languageId = LANGUAGE.KAZAKH;
        if (RUSSIAN_LANGUAGE.equals(langKey)) {
            languageId = LANGUAGE.RUSSIAN;
        } else if (ENGLISH_LANGUAGE.equals(langKey)) {
            languageId = LANGUAGE.ENGLISH;
        }
        return SessionFacadeFactory.getSessionFacade(CommonEntityFacadeBean.class).lookup(LANGUAGE.class, languageId);
    }
