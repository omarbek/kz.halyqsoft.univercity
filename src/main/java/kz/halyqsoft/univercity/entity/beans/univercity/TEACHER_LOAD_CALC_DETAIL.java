...
/**
 * @@author Omarbek
 * @created Mar 15, 2016 11:30:00 AM
 */
@Entity
public class TEACHER_LOAD_CALC_DETAIL extends AbstractEntity {

	private static final long serialVersionUID = -3171554524719148328L;

	@FieldInfo(type = EFieldType.FK_COMBO, order = 1)
    @ManyToOne
    @JoinColumns({
        @JoinColumn(name = "TEACHER_LOAD_CALC_ID", referencedColumnName = "ID")})
    private TEACHER_LOAD_CALC teacherLoadCalc;
	...
	@FieldInfo(type = EFieldType.FK_DIALOG, order = 8)
    @ManyToOne
    @JoinColumns({
	...
	public TEACHER_LOAD_CALC_DETAIL() {
	}

	public TEACHER_LOAD_CALC getTeacherLoadCalc() {
		return teacherLoadCalc;
	}

	public void setTeacherLoadCalc(TEACHER_LOAD_CALC teacherLoadCalc) {
		this.teacherLoadCalc = teacherLoadCalc;
	}
